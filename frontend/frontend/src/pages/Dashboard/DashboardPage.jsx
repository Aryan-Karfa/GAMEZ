...
function RecommendationSection({ title, games, onAdd, addingIds, navigate }) {
  ...
  return (
    <div className="space-y-sm mt-lg relative group">
      <div className="flex justify-between items-end border-b-2 border-neo-border/20 pb-xs">
        <h3 className="text-sm font-mono tracking-widest text-primary font-bold uppercase">{title}</h3>
        <div className="flex gap-xs">
          <button
            type="button"
            onClick={() => scroll('left')}
            className="border-2 border-black bg-surface hover:bg-[#eaeaea] hover:text-black h-6 w-6 flex items-center justify-center text-xs font-black cursor-pointer rounded-none active:translate-y-0.5"
          >
            &lt;
          </button>
          <button
      ...
      <div
        ref={rowRef}
        className="flex gap-md overflow-x-auto pb-sm pt-xs scrollbar-thin scroll-smooth"
      >
        {games.map((game) => {
          ...
          return (
            <div
              key={game.rawgId}
              className="w-48 bg-[#121212] border-4 border-black hover:border-accent hover:shadow-neo-accent transition-all duration-150 shrink-0 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] p-sm flex flex-col justify-between h-72 group/card select-none"
            >
              {/* Cover Art */}
              <div className="h-28 bg-surface border-2 border-black overflow-hidden relative mb-xs">
                {game.coverImage ? (
                  <img
                    src={game.coverImage}
                    alt={game.title}
                    className="w-full h-full object-cover grayscale brightness-90 group-hover/card:grayscale-0 group-hover/card:brightness-100 transition-all"
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-[10px] text-muted-text font-bold uppercase">
                    NO ART
                  </div>
                )}
                {typeof game.rating === 'number' && (
                  <div className="absolute top-1 right-1 z-10 scale-75 origin-top-right">
                    <Badge variant="accent">★ {game.rating.toFixed(1)}</Badge>
                  </div>
                )}
              </div>

              {/* Title & Platforms */}
              <div className="grow flex flex-col justify-between mb-xs">
                <h4 className="text-xs font-title tracking-wider text-primary uppercase line-clamp-2 mt-xs font-bold">
                  {game.title}
                </h4>
                <div className="text-[9px] text-muted-text uppercase font-bold tracking-widest mt-xs flex justify-between">
              ...
              <div className="flex gap-xs mt-xs">
                <button
                  ...
                  disabled={isAdding}
                  className="flex-1 border-2 border-black bg-accent text-primary text-[10px] py-1 font-bold uppercase tracking-wider hover:bg-accent-hover transition-colors rounded-none disabled:bg-muted-text disabled:text-black cursor-pointer"
                >
                  {isAdding ? 'ADD...' : 'ADD'}
                </button>
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate(`/games/${game.rawgId}`, { state: { game } });
                  }}
                  className="flex-1 border-2 border-black bg-surface text-primary text-[10px] py-1 font-bold uppercase tracking-wider hover:bg-primary hover:text-black transition-colors rounded-none cursor-pointer"
                >
                  INFO
                </button>
              </div>
...
export default function DashboardPage() {
  ...
  const { recommendations, fetchRecommendations, loadingRecommendations } = useRecommendationStore(
    useShallow((state) => ({
      recommendations: state.recommendations,
      fetchRecommendations: state.fetchRecommendations,
      loadingRecommendations: state.loading,
    }))
  );
  ...
  const handleAddToArmoryFromRecs = async (rawgId) => {
    if (!rawgId) return;
    setAddingIds((prev) => ({ ...prev, [rawgId]: true }));
    try {
      const res = await useLibraryStore.getState().addGame(rawgId, 'TO_PLAY');
      if (res.success) {
  ...
  return (
    <div className="flex flex-col gap-xl">
      ...
      {/* Discovery Hub / Recommendations */}
      <div className="mt-xl border-t-4 border-neo-border/20 pt-lg space-y-lg">
        <h2 className="text-[2.2rem] font-title tracking-widest font-normal text-accent border-b-4 border-neo-border pb-xs uppercase flex items-center justify-between">
          <span>📡 COMMANDER DISCOVERY MATRIX</span>
          <span className="h-2 w-48 warning-stripes opacity-20 border border-neo-border" />
        </h2>

        {loadingRecommendations && !recommendations ? (
          <div className="space-y-md animate-pulse">
            <div className="h-6 bg-[#2a2a2a] w-1/4" />
            <div className="flex gap-md overflow-x-auto pb-sm">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="w-48 h-72 bg-[#2a2a2a] border-4 border-black shrink-0" />
              ))}
            </div>
          </div>
        ) : (
          recommendations && (
            <div className="space-y-lg">
              <RecommendationSection
                ...
                games={recommendations.recommendedForYou}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                title="🎮 Continue Your Journey"
                games={recommendations.continueYourJourney}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                ...
                games={recommendations.becauseYouPlayed}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                title="⚡ Trending Now"
                games={recommendations.trendingNow}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                ...
                games={recommendations.recentlyReleased}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                title="💎 Hidden Gems"
                games={recommendations.hiddenGems}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                ...
                games={recommendations.upcomingReleases}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                ...
                games={recommendations.communityFavorites}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                title="🏢 From Favorite Studios"
                games={recommendations.sameStudio}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
              <RecommendationSection
                title="📂 Similar Genre Classifications"
                games={recommendations.sameGenre}
                onAdd={handleAddToArmoryFromRecs}
                addingIds={addingIds}
                navigate={navigate}
              />
