...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username.trim() || !email.trim() || !password.trim()) {
      setValidationError('ALL SIGNUP TELEMETRY VARIABLES MUST BE ALLOCATED.');
      return;
    }

    if (password !== confirmPassword) {
      setValidationError('SECURITY KEY MISMATCH. PASSWORD VERIFICATION DEVIATED.');
      return;
    }

    const result = await register(username, email, password);
    if (result && result.success) {
      useUiStore.getState().addToast('PLAYER PROFILE CREATED. BOOTING LOGIN.', 'success');
      navigate('/login');
    }
  };
  ...
  return (
    <div className="w-full max-w-140 flex flex-col gap-lg page-transition-wipe items-stretch">
      ...
      {/* Main card */}
      <Card title="PROFILE INITIALIZATION CABINET" hasScrewHeads={true}>
        {/* Header / Subtitle inside the card for BIOS look */}
        <div className="text-center mb-md border-b border-neo-border/10 pb-sm">
          <h2 className="text-2xl font-title font-bold leading-none uppercase tracking-[0.06em] text-primary">REGISTER</h2>
          <p className="text-[10px] font-sans text-muted-text mt-xs uppercase tracking-widest">
            ALLOCATE PLAYER PROFILE SECTOR IN DATABASE
          </p>
        </div>
        ...
        {activeError && (
          <div className="border-[3px] border-accent bg-accent/10 p-sm text-[11px] font-mono text-accent uppercase tracking-wider mb-md animate-shake">
            ⚠️ COMPILING FAILURE: {activeError}
          </div>
        )}
        ...
        <form className="space-y-md font-sans text-sm" onSubmit={handleSubmit}>
          ...
          <Input
            label="EMAIL ADDRESS"
            type="email"
            placeholder="warlord@gamez.local"
            value={email}
          ...
          >
            {loading ? 'WRITING PROFILE CORE...' : 'INITIALIZE PLAYER CORES'}
          </Button>
        </form>

        {/* Link to Login inside card for containment */}
        <div className="text-xs font-sans text-center pt-md border-t border-neo-border/10 mt-md uppercase tracking-wider">
          <span className="text-muted-text">PROFILE PRE-INSTALLED? </span>
          <Link to="/login" className="text-accent font-black hover:underline tracking-widest">
            SIGN IN HERE
          </Link>
        </div>
      </Card>
